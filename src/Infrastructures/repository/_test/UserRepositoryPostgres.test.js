...
describe('UserRepositoryPostgres', () => {
  ...
  describe('getIdByUsername function', () => {
    ...
    it('should return id when username is found', async () => {
      // Arrange
      await UsersTableTestHelper.addUser({id: 'user-123', username: 'dicoding' });
      const userRepositoryPostgres = new UserRepositoryPostgres(pool, {});

      // Action
      const result = await userRepositoryPostgres.getIdByUsername('dicoding');
      // Assert
      await expect(result).toStrictEqual('user-123');
    });
  });

  describe('getUsernameById function', () => {
    it('should throw InvariantError when id is not found', async () => {
      // Arrange
      const userRepositoryPostgres = new UserRepositoryPostgres(pool, {});
 
      // Action & Assert
      await expect(userRepositoryPostgres.getUsernameById('user-123')).rejects.toThrowError(InvariantError);
    });
 
    it('should return username when id is found', async () => {
      // Arrange
      await UsersTableTestHelper.addUser({id: 'user-123', username: 'dicoding' });
      const userRepositoryPostgres = new UserRepositoryPostgres(pool, {});

      // Action
      const result = await userRepositoryPostgres.getUsernameById('user-123');
      // Assert
      await expect(result).toStrictEqual('dicoding');
    });
  });
});